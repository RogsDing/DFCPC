...
class CrossCorrelationLoss(nn.Module):  # 此损失函数鼓励模型在同一样本的不同视图之间建立较低的相关性，并在不同样本之间建立较高的相关性
...
class PretrainLoss(nn.Module):  # 该损失函数用于预训练阶段的对比学习任务，在两个批次的数据之间计算损失，以促进模型学习有用的特征表示。
    """
